...
class ProductBulkTemplateService
{
    ...
    private function buildTemplate(?Vendor $vendor = null): string
    {
        ...
        if ($vendor) {
            ...
            $listColumns = [
                ...
                'C' => $subcategoryNames,
                'D' => $taxes,
                'E' => collect(['percentage', 'fixed']),
                'F' => collect(['internal', 'epos']),
                'G' => collect(['kg', 'nos', 'pieces']),
                'H' => collect(['active', 'inactive']),
            ];
            $validations = [
                'B' => 'Lists!$A$2:$A$'.max(2, $vendorNames->count() + 1),
                'D' => 'Lists!$B$2:$B$'.max(2, $categoryNames->count() + 1),
                'E' => 'Lists!$C$2:$C$'.max(2, $subcategoryNames->count() + 1),
                'F' => 'Lists!$D$2:$D$'.max(2, $taxes->count() + 1),
                'H' => 'Lists!$E$2:$E$3',
                'L' => 'Lists!$F$2:$F$3',
                'N' => 'Lists!$G$2:$G$4',
                'P' => 'Lists!$H$2:$H$3',
        ...
        $zip->addFromString('xl/worksheets/sheet1.xml', $this->sheetXml($rows, $validations));
        $zip->addFromString('xl/worksheets/sheet2.xml', $this->listsSheetXml($listColumns, $listHeadings));
    ...
    private function sheetXml(array $rows, array $validations): string
    {
        ...
        return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
            ...
            .'<cols><col min="1" max="'.count($rows[0]).'" width="22" customWidth="1"/></cols>'
            .'<sheetData>'.$sheetData.'</sheetData>'
            .'<dataValidations count="'.count($validations).'">'.$dataValidations.'</dataValidations>'
            .'</worksheet>';
    }

    private function listsSheetXml(array $columns, array $headings): string
    {
        ...
        return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
            ...
            .'<sheetViews><sheetView workbookViewId="0"/></sheetViews>'
            .'<sheetFormatPr defaultRowHeight="15"/>'
            .'<cols><col min="1" max="'.count($headings).'" width="28" customWidth="1"/></cols>'
            .'<sheetData>'.$sheetData.'</sheetData>'
            .'</worksheet>';
    }
